...
export const adjustBearingAndDistanceForWind = (
  ...
  const horizontalRangeLon =
    horizontalRange * Math.cos((Math.PI * bearingRadians) / 180) + distanceFromWindLon;
  const horizontalRangeLat =
    horizontalRange * Math.sin((Math.PI * bearingRadians) / 180) + distanceFromWindLat;

  const bearingWithWind =
    radiansToDegrees((Math.atan(horizontalRangeLat / horizontalRangeLon) * 180) / Math.PI); // radians to degrees
